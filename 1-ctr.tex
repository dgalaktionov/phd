...
\section{Algorithms} \label{sec:ctr:alg}
	...
	from the Algorithm~\ref{alg:topk_nieves}: in the \textit{line 3} we insert $(\langle 2, |V| \rangle, \langle 2,z+1 \rangle)$ into $Q$, and we 
	...
	\end{figure}
		...
		This operation was defined as $\cnt^{LR}_{a, b}(i,j)$ in Section~\ref{sec:wt}.
		...
		\begin{itemize}
			...
			To do so, we need to obtain $l'\leftarrowl+\alpha'-\alpha$ as done in \XtoY$_{Ts}$, and compute $\cnt_{t_1, |I|}(Icode^{\Psi},l,l'-1)$. This gives us the number of time instants in the range $[l..l')$ of  $Icode^{\Psi}$ that fall inside $[t_1..|I|]$. 
