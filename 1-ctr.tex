...
\section{Description}
    ...
    %We model our transportation network as a directed graph where the vertices (from now on called \textbf{nodes}) $s_i \in S$ are either street segments of a street network or stops in a public transportation network.
...
\section{Structures}
	...
	in \gls{ctr} for entries $\Psi[1,z+1]$.  %just to explain the difference of how we build $\Psi$. 
	...
	% are aligned with $\Psi$ rather than with $S$.%, and represented with a \gls{wm}.
	...
	\begin{itemize}
	 ...
	 % which is an optimal prefix code that preserves the order of the input vocabulary.
	  ...
	  %Using this codification in a \gls{wt} allows us to have a variable number of levels in the tree for each symbol,
	  ...
	  %Recall that a \gls{wm} is a grid of $n \times m$ bits. In  our
	  %case $n$ is the number of entries in the \gls{csa} and $m = \log  \sigma_t$ are the bits
	  %needed to represent the different $\sigma_t$ codes for the time instants of interest.
	  %This approach grants better compression for large alphabets when compared to the \gls{wt}, as the \gls{wm} does not need
	  %to store any kind of pointers.
	\end{itemize}

	In Figure~\ref{fig:wm}, we show both the \gls{wm} and the \gls{htwt} built on top of $Icode^{\Psi}$ from Figure~\ref{fig:tcsa}.
	...
	% %
	%\OJOFARI{ 
	%Furthermore, in the implementation of \Ttk, we used the simpler linear approach for \gls{wm}.\footnote{Since linear approach requires 
	%keeping track of the position in which each symbol starts and ends in the last level, 
	%we explicitly keep pointers that were originally used to speed-up $select$ in \cite{CNO15}.}  However, in our \gls{htwt} 
...
\section{Experiments}
	...
	%We made a custom \gls{wt} implementation with Hu-Tucker coding, where we focused in optimizing the $count$ operation. We also implemented the $top-k$ mentioned in Section~\ref{sec:tq} using a binary partition approach.
