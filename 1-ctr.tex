...
\section{Algorithms}
	...
	\end{figure}
		...
		%We refer to this operation as $\cnt_{LR}$ in Figure~\ref{fig:search}.
...
\section{Experiments}
	...
	%queries \loadTi\ and \loadTr. 
	%Yet, since \loadTi\ only needs a unique time-ID, we only randomly extracted $t_1$.
	...
	%Regarding \topK\ queries we can see that the fact of having less trips, but a larger vocabulary, make
	%\topK\ queries to perform slower at Porto than at Madrid dataset.
	...
	%This is mainly
	%noticeable for queries \XtoY$_{Ts}$\ and \XtoY$_{Tw}$.
	...
	Finally, we also include results for \topK$_T$\ and \topK$_{Ts}$\ queries in Figures~\ref{fig:madridst.tk} and \ref{fig:portost.tk}. 
